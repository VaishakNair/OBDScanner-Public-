...
/**
 * MAF sensors supported
 */
class Parameter66(context: Context) : OtherBaseParameter(
    context, pid = "661\r"
) {
    ...
    override fun processResponse(response: String) {
        ...
        if(intValue != 0) {// At least one MAF sensor is supported.
            ...
            if ((intValue and 0b0000_0001) != 0) {
                sb.append("${context.getString(R.string.maf_a_supported)}\n")
            }
            if ((intValue and 0b0000_0010) != 0) {
                sb.append("${context.getString(R.string.maf_b_supported)}\n")
            }
