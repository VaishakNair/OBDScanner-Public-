 ...
 */
class Parameter3D(context: Context, viewModel: ViewModel?, gaugeType: String? = null) : BaseParameter(
    context, viewModel, pid = "3D1\r", gaugeUnit = "Catalyst Temp - B2S1(°C)",
    ticks = listOf(-40f, 1000f, 2000f, 3000f, 4000f, 5000f, 6600f),
    min = -40f, max = 6600f, unitTextSize = 33f,
    parameterName = "Catalyst temperature - B2S1", gaugeType = gaugeType
) {
    ...
    init {
        if(PreferenceManager.getDefaultSharedPreferences(context).getString(
            ...
            gauge.unit = "Catalyst Temp - B2S1(°F)"
    ...
    override fun calculateValue(response: Float) {
        ...
        Log.i(APP_NAME, "Catalyst temperature (Bank 2 Sensor 1) ($unit): $value")
    }

    override fun toString(): String {
        return "${context.getString(R.string.cat_temp_b2_s1)} ($unit): $valueString"
    }
}