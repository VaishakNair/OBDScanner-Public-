 ...
 */
class Parameter3E(context: Context, viewModel: ViewModel?, gaugeType: String? = null) : BaseParameter(
    context, viewModel, pid = "3E1\r", gaugeUnit = "Catalyst Temp - B1S2(°C)",
    ticks = listOf(-40f, 1000f, 2000f, 3000f, 4000f, 5000f, 6600f),
    min = -40f, max = 6600f, unitTextSize = 33f,
    parameterName = "Catalyst temperature - B1S2", gaugeType = gaugeType
) {
    ...
    init {
        if(PreferenceManager.getDefaultSharedPreferences(context).getString(
            ...
            gauge.unit = "Catalyst Temp - B1S2(°F)"
    ...
    override fun calculateValue(response: Float) {
        ...
        Log.i(APP_NAME, "Catalyst temperature (Bank 1 Sensor 2) ($unit): $value")
    }

    override fun toString(): String {
        return "${context.getString(R.string.cat_temp_b1_s2)} ($unit): $valueString"
    }
}